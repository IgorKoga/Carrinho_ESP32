...
// Ajuste para testar 0° como centro (se o servo limitar em zero, os valores negativos podem não responder)
int currentServoAngle = 0;
int servoCenterAngle  = 0;   // Posição central / reto (0°)
int servoMinAngle     = -45; // Limite máximo de curva à esquerda (graus)
int servoMaxAngle     = 45;  // Limite máximo de curva à direita (graus)
...
void setup() {
  ...
  // Configura o Flash com PWM para evitar alto consumo (brownout)
  PWM_ATTACH(FLASH_GPIO_NUM, PWM_FREQ, PWM_RES, PWM_CHAN_FLASH);
  PWM_WRITE(FLASH_GPIO_NUM, PWM_CHAN_FLASH, 0); // Flash desligado
  ...
  // Endpoint para ligar/desligar o Flash (Usando PWM para evitar desconexão por falta de energia)
  server.on("/api/flash", HTTP_GET, []() {
    if (server.hasArg("state")) {
      String state = server.arg("state");
      if (state == "on" || state == "1" || state == "true") {
        isFlashOn = true;
        // PWM 10 (de 255) ilumina bastante e não derruba a tensão do ESP32 (Brownout)
        PWM_WRITE(FLASH_GPIO_NUM, PWM_CHAN_FLASH, 10);
      } else {
        isFlashOn = false;
        PWM_WRITE(FLASH_GPIO_NUM, PWM_CHAN_FLASH, 0);
      }
    }
    server.send(200, "application/json", "{\"flash\":" + String(isFlashOn ? "true" : "false") + "}");
  });
  ...
  server.on("/api/telemetry", HTTP_GET, []() {
    ...
    json += "\"flash\":" + String(isFlashOn ? "true" : "false");
    json += "}";
